 ...
 * @constructor
 */
var LogFileWatcher = function LogFileWatcher(logFilePath, pattern, fieldNames, opts) {
...
util.inherits(LogFileWatcher, EventEmitter);

/**
 * Start watching the log file. If the log file is already been watching, it does nothing.
 */
LogFileWatcher.prototype.start = function start() {
  var self = this;
  if (self.watcher) {
    return;
  }
        ...
        lines.forEach(function(line) {
          ...
          if (fieldsValues === null) {
            ...
            if (self.allowPatternViolations && logs.length) {
              ...
              self.emit('error', new Error('LogFileWatcher: log line does not follow the given pattern: ' + line));
              return;
            }
          }
 ...
 */
LogFileWatcher.prototype.stop = function stop() {
